...
class DBModuleManager(reactContext: ReactApplicationContext) :
  ...
  fun findPossibleWords(
    ...
    try {
      ...
      val words =
        gson.fromJson<ArrayList<String>>(allWords, object : TypeToken<ArrayList<String>>() {}.type)
          .toMutableList()
      val selectedLetters =
      ...
      words.forEach { word ->
        ...
        val _letters = selectedLetters
          .filter { letter ->
            letter != LETTER_SOAP &&
              !letter.contains(LETTER_SOAP_PLACEHOLDER) &&
              !letter.contains(LETTER_INDEX_SEPARATOR)
          }
          .toMutableList()
        ...
        word.map { it.uppercase() }.forEachIndexed { index, char ->
          ...
          if (missingChar) {
            val customSoapLetterAvailableIndexes = custom_soap_letters
              .mapIndexed { customIndex, customSoapLetters ->
      ...
      filterWords.forEach { result.pushString(it) }
      promise.resolve(result)
    } catch (error: Exception) {
      promise.reject("DB_SEARCH_ERROR", error.message, error)
    }
  }
}
